...
export async function POST(request: Request) {
  ...
  if (
    ...
    return Response.json(
      { error: 'Đánh giá cần 1–5 sao và nội dung từ 5 đến 1000 ký tự.' },
      { status: 400 },
    );
  const db = getDb();
  ...
  if (!purchased)
    return Response.json(
      {
        error: 'Chỉ khách đã hoàn thành đơn có sản phẩm này mới được đánh giá.',
      },
      { status: 403 },
    );
  const now = Date.now();
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: { rating, content, updatedAt: now },
    });
