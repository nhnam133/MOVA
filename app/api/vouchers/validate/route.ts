...
export async function GET(request: Request) {
  ...
  if (!code || !Number.isInteger(subtotal) || subtotal < 0)
    return Response.json(
      { error: 'Mã hoặc giá trị đơn không hợp lệ.' },
      { status: 400 },
    );
  const [voucher] = await getDb()
    .select({ value: vouchers.value, minimum: vouchers.minimumOrderValue })
  ...
  if (!voucher)
    return Response.json(
      { error: 'Voucher không tồn tại, đã dùng hoặc đã hết hạn.' },
      { status: 404 },
    );
  if (subtotal < voucher.minimum)
    return Response.json(
      {
        error: `Đơn cần tối thiểu ${voucher.minimum.toLocaleString('vi-VN')}đ trước voucher.`,
      },
      { status: 409 },
    );
