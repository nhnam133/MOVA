...
export default async function ProductsPage({
  ...
  let visibleProducts = products.filter((product) => {
    ...
    if (params['danh-muc'] && product.categorySlug !== params['danh-muc'])
      return false;
    ...
    if (
      ...
      !product.variants.some(
        (variant) =>
          normalize(variant.color) === normalize(params.color!) &&
  ...
  visibleProducts = [...visibleProducts].sort((a, b) =>
    params.sort === 'gia-tang'
      ? a.price - b.price
      : params.sort === 'gia-giam'
        ? b.price - a.price
        : 0,
  );
  const pageCount = Math.max(1, Math.ceil(visibleProducts.length / 12));
  const page = Math.min(
    pageCount,
  ...
  const activeName = categories.find(
    (category) => category.slug === params['danh-muc'],
  )?.name;
  ...
  const field =
    'h-11 w-full rounded-xl border border-black/15 bg-white px-3 text-sm outline-none focus:border-black focus:ring-2 focus:ring-[#dfff00]';
  ...
  return (
    <main className="min-h-screen bg-[#f6f6f2]">
      ...
      <section className="bg-black px-4 py-14 text-white sm:px-8 lg:px-12 lg:py-20">
        <div className="mx-auto max-w-[1384px]">
          ...
          <p className="mt-5 max-w-xl text-sm leading-7 text-white/55">
            Lọc đúng màu và size trên cùng một biến thể. Giá và tồn kho được
            kiểm tra lại khi đặt hàng.
          </p>
        </div>
      </section>
      <section className="mx-auto max-w-[1480px] px-4 py-10 sm:px-8 lg:px-12 lg:py-16">
        <form
          action="/san-pham"
          className="mb-8 rounded-2xl border border-black/10 bg-white p-4 shadow-sm sm:p-5"
        >
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-7">
            <label className="relative lg:col-span-2">
              <span className="sr-only">Tên sản phẩm</span>
            ...
            <label>
              <span className="sr-only">Đối tượng</span>
              <select
                name="gender"
                defaultValue={params.gender ?? ''}
                className={field}
              >
                <option value="">Nam / Nữ / Unisex</option>
            ...
            <button className="flex h-11 cursor-pointer items-center justify-center gap-2 rounded-xl bg-black px-4 text-sm font-black text-white transition hover:bg-neutral-800">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-3 lg:max-w-2xl">
            ...
            <label>
              ...
              <select
                name="sort"
                defaultValue={params.sort ?? 'moi-nhat'}
                className={field}
              >
        ...
        <div className="mb-7 flex items-center justify-between">
          <p className="text-sm text-neutral-500">
            <b className="text-black">{visibleProducts.length}</b> sản phẩm phù
            hợp
          </p>
        </div>
        {visibleProducts.length > 0 ? (
          ...
          <div className="rounded-2xl border border-black/10 bg-white px-6 py-20 text-center">
            ...
            <p className="mt-3 text-sm text-neutral-500">
              Thử bỏ bớt điều kiện hoặc tìm bằng từ khóa khác.
            </p>
            <Link
              href="/san-pham"
        ...
        {pageCount > 1 && (
          <Pagination aria-label="Trang danh sách sản phẩm" className="mt-10">
            <PaginationContent>
              {page > 1 && (
                <PaginationItem>
                  <PaginationPrevious
