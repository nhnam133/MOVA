...
export function ProductForm({ categories }: { categories: { id: string; name: string }[] }) {
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    if (!response.ok) { setMessage(result.error || 'Không thể thêm sản phẩm.'); setSubmitting(false); return; }
    window.location.reload();
  }
  const input = 'mt-2 h-11 w-full border border-black/25 bg-white px-3 text-sm outline-none focus:border-black';
  return (
    <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-xs font-bold uppercase tracking-wider sm:col-span-2">Ảnh sản phẩm<input name="image" required type="file" accept="image/avif,image/jpeg,image/png,image/webp" className="mt-2 block w-full border border-dashed border-black/30 bg-white p-4 text-sm" /></label>
      <label className="flex items-center gap-2 text-sm font-bold sm:col-span-2"><input name="featured" type="checkbox" />Hiển thị nổi bật</label>
      {message && <p role="alert" className="text-sm text-red-600 sm:col-span-2">{message}</p>}
      <button disabled={submitting} className="flex h-12 items-center justify-center gap-2 bg-black text-xs font-black uppercase tracking-wider text-white sm:col-span-2">{submitting ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}Thêm sản phẩm</button>
    </form>
  );
}
