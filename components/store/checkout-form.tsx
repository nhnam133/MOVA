...
import { LoaderCircle, Smartphone, Truck } from 'lucide-react';
...
export function CheckoutForm({ customerName, customerEmail }: { customerName: string; customerEmail: string }) {
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/orders', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({
      items, paymentMethod, recipientName: data.get('recipientName'), recipientPhone: data.get('recipientPhone'), addressLine: data.get('addressLine'), ward: data.get('ward'), district: data.get('district'), province: data.get('province'), note: data.get('note'),
    }) });
  ...
  return (
    <form onSubmit={submit} className="grid gap-8 lg:grid-cols-[1fr_390px]">
      ...
      <aside className="h-fit border border-black bg-black p-6 text-white lg:sticky lg:top-28"><h2 className="text-xl font-black uppercase">Đơn hàng</h2><div className="my-6 space-y-3 border-y border-white/20 py-5 text-sm">{items.map((item) => { const product = catalog.find((entry) => entry.slug === item.productSlug); return product ? <div key={item.sku} className="flex justify-between gap-4"><span className="text-white/65">{product.name} × {item.quantity}</span><span>{formatMoney(product.price * item.quantity)}</span></div> : null; })}</div><dl className="space-y-3 text-sm"><div className="flex justify-between"><dt className="text-white/55">Tạm tính</dt><dd>{formatMoney(totals.subtotal)}</dd></div><div className="flex justify-between"><dt className="text-white/55">Vận chuyển</dt><dd>{totals.shipping === 0 ? 'Miễn phí' : formatMoney(totals.shipping)}</dd></div><div className="flex justify-between border-t border-white/20 pt-5 text-lg font-black"><dt>Tổng cộng</dt><dd className="text-[#eaff2f]">{formatMoney(totals.total)}</dd></div></dl>{error && <p role="alert" className="mt-5 bg-red-500/15 p-3 text-sm text-red-200">{error}</p>}<button disabled={submitting} className="mt-6 flex h-14 w-full items-center justify-center gap-2 bg-[#eaff2f] text-sm font-black uppercase tracking-wider text-black disabled:opacity-50">{submitting && <LoaderCircle className="h-4 w-4 animate-spin" />}Đặt hàng</button><p className="mt-4 text-center text-[11px] leading-5 text-white/45">Điểm được cộng khi đơn hoàn tất và đã thanh toán.</p></aside>
    </form>
  );
}
